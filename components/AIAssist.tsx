...
const AIAssist: React.FC<AIAssistProps> = ({ isOpen, onClose, logo }) => {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: 'Welcome to the BROHUBS Intelligence Hub. How can I assist with your broadcast production today?' }
  ]);
  ...
  const handleSend = async () => {
    ...
    setMessages(prev => [...prev, { role: 'model', text: response || "I'm sorry, I couldn't process that request at the moment." }]);
    setIsLoading(false);
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-white font-bold tracking-tight uppercase flex items-center gap-1">
                  BRO<span className="text-[#ccff00]">HUBS</span> SUPPORT
                </h3>
                <p className="text-[10px] text-[#ccff00] font-black tracking-widest uppercase">System Online</p>
              </div>
            </div>
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-zinc-900 border border-white/5 p-4 rounded-2xl rounded-tl-none flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-400 font-medium">BROHUBS is thinking...</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="p-6 border-t border-white/10 bg-zinc-950">
            <div className="relative flex items-center">
              <input
                ...
                placeholder="Ask about BROHUBS features..."
