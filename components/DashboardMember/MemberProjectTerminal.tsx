    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm font-medium">Access your private operational projects.</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <History size={14} />
            <span>History Logs ({historyProjects.length})</span>
          </button>
        ...
        {projects.length === 0 ? (
          <div className="h-full rounded-[32px] border-2 border-dashed border-white/10 bg-zinc-900/20 flex flex-col items-center justify-center relative overflow-hidden group hover:border-white/20 transition-colors py-12 mb-12">
            ...
            <div className="relative z-10 flex flex-col items-center justify-center p-8 text-center">
                ...
                <h3 className="text-[10px] font-black text-zinc-600 tracking-[0.2em] uppercase mb-1 group-hover:text-zinc-400 transition-colors">NO PROJECTS ACTIVE</h3>
                <p className="text-[9px] text-zinc-700 max-w-xs font-medium leading-relaxed">Initialize a broadcast to begin.</p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {projects.map((project) => (
              <div key={project.id} className="group relative bg-zinc-900/50 border border-white/10 rounded-[32px] p-8 hover:border-[#ccff00]/40 transition-all hover:bg-zinc-900 animate-in zoom-in-95 duration-300">
                ...
                <p className="text-sm text-zinc-500 line-clamp-2 mb-6 font-medium leading-relaxed">{project.description || "Operational broadcast instance."}</p>
                <div className="flex items-center justify-between pt-6 border-t border-white/5">
                  ...
                  <button onClick={() => onOpenProject(project)} className="flex items-center gap-2 text-[10px] font-black text-[#ccff00] uppercase tracking-widest group-hover:translate-x-1 transition-transform">OPEN PROJECT <ExternalLink size={12} /></button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="animate-in slide-in-from-bottom-8 duration-700 mt-auto">
        <div className="mb-6 border-t border-white/5 pt-6">
            <h2 className="text-2xl font-bold tracking-tight text-white mb-1">Master Asset Hub</h2>
            <p className="text-gray-500 text-xs font-medium">Select a game module to access assets.</p>
        </div>
            ...
            {games.map((game) => (
                <button
                    ...
                    className={`
                        ...
                        transform -skew-x-12 transition-all duration-300 border shrink-0
                        ${game.active 
                ...
                >
                    <span className={`
                        transform skew-x-12 text-[12px] font-black tracking-widest uppercase transition-colors
                        ${activeHubGameId === game.id 
                            ? 'text-black' 
            ...
            <div className="mb-12 p-8 rounded-[40px] bg-zinc-900/40 border border-white/5 animate-in slide-in-from-top-4 duration-500 relative overflow-hidden">
                ...
                <div className="relative z-10 flex items-center justify-between mb-8 pb-4 border-b border-white/5">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-xl font-black italic text-white uppercase tracking-tight">Master <span className="text-[#ccff00]">Asset Hub</span></h3>
                ...
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredHubAssets.length > 0 ? (
                        ...
                        <div className="col-span-full py-12 text-center border border-dashed border-white/10 rounded-3xl opacity-40">
                            <p className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em]">NO ASSETS CONFIGURED FOR THIS MODULE</p>
                        </div>
                    )}
                </div>
            </div>
      ...
      {isProjectModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-[#050505] border border-white/5 rounded-[48px] p-10 animate-in zoom-in-95 duration-300 shadow-[0_0_100px_rgba(0,0,0,0.9)] overflow-hidden">
            ...
            <div className="mb-10">
                <div className="flex items-center gap-2 mb-2"><div className="w-1.5 h-1.5 bg-[#ccff00] rounded-full animate-pulse" /><span className="text-[10px] font-black text-zinc-500 tracking-[0.4em] uppercase">Deployment Unit</span></div>
                <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter">New Operational <span className="text-[#ccff00]">Instance</span></h2>
            </div>
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                    <label className="text-[9px] font-black text-zinc-600 tracking-widest uppercase ml-4">Identification</label>
                    <input type="text" value={projectIdentifier} onChange={(e) => setProjectIdentifier(e.target.value)} placeholder="Project Identifier" className="w-full bg-[#111] border border-white/5 rounded-3xl px-8 py-5 text-white placeholder:text-zinc-700 outline-none focus:border-[#ccff00]/30 transition-all font-bold text-lg" />
                </div>
                <div className="space-y-2">
                    <label className="text-[9px] font-black text-zinc-600 tracking-widest uppercase ml-4">Module Selection</label>
                ...
                <div className="space-y-2">
                    <label className="text-[9px] font-black text-zinc-600 tracking-widest uppercase ml-4">Description (Optional)</label>
                    <textarea value={operationalDescription} onChange={(e) => setOperationalDescription(e.target.value)} placeholder="Brief description of broadcast mission..." rows={3} className="w-full bg-[#111] border border-white/5 rounded-3xl px-8 py-5 text-white placeholder:text-zinc-700 outline-none focus:border-[#ccff00]/30 transition-all font-bold text-base resize-none" />
                </div>
              </div>
              <button onClick={handleAuthorize} disabled={!projectIdentifier.trim() || !selectedGameId} className="w-full bg-[#ccff00] text-black py-7 rounded-[32px] font-black text-sm tracking-[0.3em] uppercase hover:scale-[1.02] active:scale-95 transition-all shadow-[0_0_50px_rgba(204,255,0,0.3)] mt-2 disabled:opacity-30 disabled:hover:scale-100 disabled:cursor-not-allowed">AUTHORIZE DEPLOYMENT</button>
            </div>
          </div>
        </div>
      )}
      ...
      {projectToDelete && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-[#050505] border border-red-500/20 rounded-[40px] p-10 animate-in zoom-in-95 duration-300 shadow-[0_0_80px_rgba(239,68,68,0.1)] flex flex-col items-center text-center">
            ...
            <h2 className="text-2xl font-black text-white italic uppercase tracking-tighter mb-2">Confirm <span className="text-red-500">Archive</span></h2>
            <p className="text-[10px] font-black text-zinc-600 tracking-[0.2em] uppercase mb-8 leading-relaxed max-w-[280px]">Are you sure you want to archive <span className="text-white">"{projectToDelete.name}"</span>? It will be moved to History Logs.</p>
            <div className="grid grid-cols-2 gap-4 w-full">
                <button onClick={() => setProjectToDelete(null)} className="py-5 rounded-2xl bg-zinc-900 border border-white/5 text-zinc-400 hover:text-white transition-all text-[10px] font-black tracking-[0.2em] uppercase">CANCEL</button>
                <button onClick={confirmDelete} className="py-5 rounded-2xl bg-red-600 text-white font-black text-[10px] tracking-[0.2em] uppercase shadow-[0_0_30px_rgba(220,38,38,0.3)] hover:scale-105 transition-all">YES, ARCHIVE</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isHistoryModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-[#050505] border border-white/5 rounded-[48px] p-10 animate-in zoom-in-95 duration-300 shadow-[0_0_100px_rgba(0,0,0,1)] overflow-hidden">
            ...
            <div className="mb-10">
                <div className="flex items-center gap-2 mb-2"><div className="w-1.5 h-1.5 bg-[#ccff00] rounded-full" /><span className="text-[10px] font-black text-zinc-500 tracking-[0.4em] uppercase">Archive Terminal</span></div>
                <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter">History <span className="text-[#ccff00]">Logs</span></h2>
            </div>
            <div className="space-y-4 max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
                {historyProjects.length === 0 ? (
                    <div className="py-20 text-center opacity-30 border border-dashed border-white/10 rounded-3xl"><History size={48} className="mx-auto mb-4" /><p className="text-xs font-black tracking-widest uppercase">No Archived Projects</p></div>
                ) : (
                    historyProjects.map((project) => (
                        <div key={project.id} className="bg-[#111] border border-white/5 rounded-2xl p-6 flex items-center justify-between group hover:border-[#ccff00]/30 transition-all">
                            ...
                            <div className="flex items-center gap-3">
                                <button onClick={() => restoreProject(project.id)} className="flex items-center gap-2 px-4 py-2 bg-[#ccff00]/10 border border-[#ccff00]/20 rounded-xl text-[9px] font-black text-[#ccff00] uppercase tracking-widest hover:bg-[#ccff00] hover:text-black transition-all"><RotateCcw size={12} /> RESTORE</button>
                                <button onClick={() => permanentDelete(project.id)} className="p-3 bg-red-500/10 border border-red-500/20 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all" title="Delete Permanently"><Trash2 size={14} /></button>
                            </div>
                        </div>
                    ))
                )}
            </div>
            <div className="mt-8 pt-8 border-t border-white/5 flex justify-between items-center"><p className="text-[9px] font-black text-zinc-700 uppercase tracking-widest">ARCHIVED NODES: {historyProjects.length}</p><button onClick={() => setIsHistoryModalOpen(false)} className="text-[9px] font-black text-[#ccff00] uppercase tracking-widest hover:underline">CLOSE ARCHIVE</button></div>
          </div>
        </div>
      )}
