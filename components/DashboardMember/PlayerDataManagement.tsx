...
const PlayerDataManagement: React.FC<PlayerDataManagementProps> = ({ players, editTeamName, onUpdatePlayers, onClose }) => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, id: string | 'team') => {
    ...
    if (file) {
      if (file.size > 1024 * 1024 * 2) {
        alert("IMAGE TOO LARGE. PLEASE USE FILES UNDER 2MB.");
        return;
      }
  ...
  const handleDeploy = () => {
    ...
    if (hasTeamValidationError) {
      alert("PLEASE ENTER TEAM NAME FOR ALL PLAYERS (OR SPECIFY A MASTER TEAM NAME).");
      return;
    }

    if (processedPlayers.length === 0) {
      alert("PLEASE ADD AT LEAST ONE PLAYER OR IMPORT NICKNAMES.");
      return;
    }
  ...
  const parseAndApplyBulkText = () => {
    if (!bulkText.trim()) {
      alert("PLEASE ENTER NICKNAMES TO PARSE.");
      return;
    }
    ...
    setBulkText('');
    setImportFeedback({ type: 'success', message: `${names.length} PLAYERS SUCCESSFULLY GENERATED & APPENDED!` });
    setTimeout(() => {
      setImportFeedback(null);
      setActiveTab('MANUAL');
    }, 2000);
  ...
  const handleCSVImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (rows.length === 0) {
          setImportFeedback({ type: 'error', message: "EMPTY OR INVALID CSV FILE." });
          return;
        }
        ...
        rows.forEach(row => {
          ...
          const image = row['PLAYER IMAGE URL'] || row['AVATAR URL'] || row['AVATAR'] || row['IMAGE'] || row['PLAYER IMAGE'] || '';
        ...
        if (newEntries.length === 0) {
          setImportFeedback({ type: 'error', message: "COULD NOT EXTRACT VALID PLAYER NAMES." });
          return;
        }
        ...
        setImportFeedback({ 
          type: 'success', 
          message: `SUCCESS! PARSED ${newEntries.length} PLAYERS ACROSS ${uniqueTeamsInImport.length} TEAMS: ${uniqueTeamsInImport.join(', ').toUpperCase()}`
        });
        ...
        setImportFeedback({ type: 'error', message: "ERROR PARSING CSV TEMPLATE. CHECK FORMAT." });
      }
    };
            ...
            <div className="flex items-center gap-3 border-b border-white/10 pb-3 mb-1">
               ...
               <div className="flex-1">
                 ...
                 <input className="text-[#ccff00] font-mono text-[10px] bg-transparent w-full outline-none" value={`[${teamData.abbr || '---'}]`} onChange={e => handleTeamEdit(teamData.name, 'teamAbbreviation', e.target.value.replace(/[\[\]]/g, ''))} />
                  <input className="text-zinc-500 font-mono text-[8px] bg-transparent w-full outline-none border-b border-transparent focus:border-white/10 focus:text-zinc-300 transition-colors mt-0.5" value={teamData.logo} onChange={e => handleTeamEdit(teamData.name, 'teamLogo', e.target.value)} placeholder="🔗 PASTE LOGO LINK" />
               </div>
            </div>
                    ...
                    {editingPlayerId === player.id ? (
                        <div className="flex flex-col gap-1.5 flex-1 bg-black/50 p-2 rounded-lg border border-white/15">
                            <div className="flex items-center gap-1.5">
                                <span className="text-zinc-500 text-[8px] tracking-wider shrink-0">NAME:</span>
                                <input 
                            ...
                            <div className="flex items-center gap-1.5">
                                <span className="text-zinc-500 text-[8px] tracking-wider shrink-0">PHOTO URL:</span>
                                <input 
        ...
        <div className="p-6 md:p-8 pb-4 flex justify-between items-start shrink-0">
          <div>
            <h2 className="text-xl md:text-3xl font-[1000] italic text-white uppercase tracking-tighter leading-none mb-1 text-left">
              {editTeamName ? 'UPDATE' : 'PLAYER'} <span className={editTeamName ? 'text-blue-500' : 'text-[#ccff00]'}>DATA</span>
            </h2>
            <p className="text-[7px] md:text-[9px] font-black text-zinc-600 uppercase tracking-[0.4em] text-left">ROSTER CONFIGURATION</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 pt-0 custom-scrollbar space-y-6">
          <section className="bg-zinc-900/40 p-5 md:p-6 rounded-[24px] md:rounded-[32px] border border-white/5 shadow-inner relative overflow-hidden">
            <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-[10px] font-black text-white uppercase tracking-[0.3em]">TEAM_IDENTITY</h3>
            </div>
            ...
            <div className={`flex flex-col md:flex-row gap-4 md:gap-6 items-start transition-all duration-300 ${activeTab === 'IMPORT_EXCEL' && !editTeamName ? 'opacity-20 pointer-events-none select-none blur-[1.5px]' : ''}`}>
              ...
              <div className="shrink-0 space-y-1.5 w-full md:w-auto">
                <div className="flex justify-between items-center px-1">
                    <span className="text-[7px] font-black text-zinc-500 uppercase tracking-widest">REC: 512x512</span>
                    {teamLogo && !isSaved && (
                        <button onClick={() => resetImage('team')} className="text-[7px] font-black text-[#ccff00] hover:text-white flex items-center gap-1 uppercase tracking-widest"><RotateCcw size={8}/> RESET</button>
                    )}
                </div>
            ...
            {activeTab === 'IMPORT_EXCEL' && !editTeamName && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/30 backdrop-blur-[1px] p-6 text-center animate-in fade-in duration-300">
                <div className="bg-[#ccff00]/10 border border-[#ccff00]/30 text-[#ccff00] px-4 py-2.5 rounded-2xl flex items-center gap-2 shadow-2xl mb-1.5">
                  ...
                  <span className="text-[9px] font-black uppercase tracking-[0.25em]">IDENTITY LOCKED</span>
                </div>
                <p className="text-[8.5px] font-black text-white uppercase tracking-widest leading-normal mb-1">
                  PENGISIAN TEAM SECARA MANUAL DINONAKTIFKAN
                </p>
                <p className="text-[7.5px] font-bold text-zinc-500 uppercase tracking-widest max-w-md leading-normal">
                  Nama & Logo Team otomatis dibaca dari file Excel/CSV baris demi baris secara otomatis! Silakan langsung upload file Anda di bawah.
                </p>
              </div>
            )}
          </section>
          ...
          <section className="space-y-4">
            <div className="flex bg-black/60 p-1 rounded-2xl border border-white/10 shadow-2xl max-w-2xl mx-auto">
              ...
              >
                <ClipboardCheck size={12} /> BULK NICKNAMES
              </button>
              <button 
                type="button"
            ...
            <section className="bg-zinc-900/20 border border-white/5 rounded-[32px] p-6 space-y-6 animate-in fade-in duration-200">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <h4 className="text-xs font-black text-white uppercase tracking-wider">BULK NICKNAME PARSER</h4>
                  <p className="text-[8px] font-bold text-zinc-500 uppercase tracking-widest">MASUKKAN BANYAK PEMAIN SEKALIGUS</p>
                </div>
              </div>

              <div className="space-y-4">
                <div className="space-y-2 text-left">
                  <label className="text-[8px] font-black text-zinc-600 uppercase tracking-[0.2em] ml-2">Daftar Nickname (Pisahkan per baris atau dengan koma)</label>
                  <textarea 
                    value={bulkText}
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-black/40 border border-white/5 rounded-2xl">
                  <div className="flex items-center gap-3">
                    ...
                    <label htmlFor="appendMode" className="text-[8.5px] font-black text-zinc-400 uppercase tracking-widest cursor-pointer select-none">
                      Gabungkan dengan Roster Aktif ({playerEntries.length} player)
                    </label>
                  </div>
            ...
            <section className="bg-zinc-900/20 border border-white/5 rounded-[32px] p-6 space-y-6 animate-in fade-in duration-200">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <h4 className="text-xs font-black text-white uppercase tracking-wider">Excel / CSV Spreadsheet Loader</h4>
                    <p className="text-[8px] font-bold text-[#ccff00] uppercase tracking-widest flex items-center gap-1.5">
                      ...
                      MENDUKUNG IMPORT HINGGA BANYAK TEAM SEKALIGUS!
                    </p>
                  </div>
                </div>
                ...
                >
                  <Download size={12} /> DOWNLOAD FORMAT EXCEL
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="relative group min-h-[160px] rounded-2xl border-2 border-dashed border-white/10 hover:border-[#ccff00]/30 transition-all bg-black/40 flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <p className="text-[9px] font-black text-white uppercase tracking-widest mb-1">UPLOAD EXCEL SPREADSHEET (.CSV)</p>
                  <p className="text-[7px] font-bold text-zinc-600 uppercase">Seret file di sini atau klik untuk memilih file</p>
                </div>
                ...
                <div className="p-5 rounded-2xl border border-white/5 bg-black/20 flex flex-col justify-between text-left">
                  <div>
                    <h5 className="text-[8px] font-black text-zinc-500 tracking-[0.2em] uppercase mb-1 italic">ACCEPTED EXCEL COLUMNS</h5>
                    <p className="text-[7.5px] font-semibold text-zinc-400 uppercase tracking-widest mb-3 leading-tight">
                      Sistem membaca header kolom berikut di Excel/CSV secara otomatis:
                    </p>
                    ...
                    <div className="space-y-1.5 border-t border-white/5 pt-3">
                      ...
                      <p className="text-[8px] font-semibold text-zinc-400 leading-normal">
                        Isi kolom <span className="text-white font-black">TEAM NAME</span> dengan nama tim berbeda di setiap baris pemain untuk mengelompokkan mereka ke beberapa tim secara otomatis. Form <span className="text-[#ccff00] font-black">Team Identity</span> di atas <span className="text-blue-400 font-black">tidak wajib diisi</span> jika Anda memakai fitur ini.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-white/5 flex items-center gap-3">
                    ...
                    <label htmlFor="appendModeCSV" className="text-[8.5px] font-black text-zinc-400 uppercase tracking-widest cursor-pointer select-none">
                      Gabungkan Roster Terbaca Dengan Roster Aktif
                    </label>
                  </div>
                </div>
              </div>
              ...
              {playerEntries.length > 0 && (
                <div className="pt-6 border-t border-white/5">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="text-[10px] font-black text-white uppercase tracking-[0.3em]">IMPORT_PREVIEW ({playerEntries.length})</h3>
                    </div>
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between border-t border-white/5 pt-6">
                <div className="flex items-center gap-2">
                  <User size={12} className="text-[#ccff00]" />
                  <h3 className="text-[10px] font-black text-white uppercase tracking-[0.3em]">OPERATIONAL_ROSTER ({playerEntries.length})</h3>
                </div>
                {!isSaved && (
                  <button onClick={handleAddPlayer} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#ccff00]/10 border border-[#ccff00]/20 text-[#ccff00] text-[9px] font-black tracking-widest uppercase hover:bg-[#ccff00] hover:text-black transition-all">
                    <Plus size={12} strokeWidth={3} /> ADD PLAYER
                  </button>
                )}
              </div>
                  ...
                  <div key={player.id} className="bg-zinc-900/40 border border-white/5 rounded-3xl p-4 flex gap-4 items-center group hover:border-[#ccff00]/20 transition-all shadow-sm">
                    <div className="shrink-0 space-y-1">
                      <div className="flex justify-between items-center px-0.5">
                          <span className="text-[6px] font-black text-zinc-600 uppercase">800x1200</span>
                    ...
                    <div className="flex-1 space-y-1 text-left">
                      <div className="flex items-center justify-between">
                        <label className="text-[7px] font-black text-zinc-700 uppercase tracking-widest ml-1">NICKNAME</label>
                        {(player.kills !== undefined || player.role) && (
                          <span className="text-[6.5px] font-black bg-[#ccff00]/10 text-[#ccff00] px-1.5 py-0.5 rounded uppercase tracking-wider">
                            STATS LOADED: {player.kills || 0}K | {player.role || 'PLAYER'}
                          </span>
                        )}
                      </div>
                      <input 
