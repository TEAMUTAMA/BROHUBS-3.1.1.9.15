...
const Navbar: React.FC<NavbarProps> = ({ 
  ...
  const navItems = ['EXPLORE', 'BROADCAST', 'PRICING', 'STUDIO'];
  ...
  const handleLogin = async () => {
    ...
    try {
        ...
        setLoginError('SYSTEM ERROR: CONNECTION FAILED');
    } finally {
  ...
  const handleSubmitReset = () => {
    ...
    setTimeout(() => {
        ...
        setIsResetSent(false);
        setResetEmail('');
        setLoginError('REQUEST SENT TO ADMIN');
    }, 2000);
  };
      ...
      <nav className="fixed top-0 left-0 w-full z-50 bg-black/80 backdrop-blur-md border-b border-white/5 px-4 md:px-6 py-4 flex items-center justify-between">
          ...
          {navItems.map((item) => (
            <a
              ...
              className={`text-xs font-bold tracking-widest text-gray-400 hover:text-white transition-colors relative overflow-hidden group/link ${
                item === 'PRICING' ? 'border-b-2 border-transparent hover:border-[#ccff00]' : ''
              }`}
            >
              {item}
        ...
        <div className="flex items-center gap-2 md:gap-6">
          ...
          >
            ...
            <span className="text-[10px] md:text-xs font-bold tracking-widest text-white uppercase group-hover:text-[#ccff00] transition-colors whitespace-nowrap">AI Assist</span>
          </button>
                ...
                <>
                    ...
                    <div className="space-y-4 animate-in slide-in-from-right duration-300 delay-100">
                        ...
                        <button 
                            ...
                            `}
                        >
                             {isResetSent ? 'REQUEST SENT' : 'SEND REQUEST'}
