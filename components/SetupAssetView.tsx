             ...
             {fraggers.map((f, i) => (
                 <div key={i} className="w-64 h-96 bg-[#ccff00] relative flex flex-col items-center pt-10 shadow-2xl">
                     ...
                     <div className="mt-4 bg-black/10 px-4 py-2 rounded text-center">
                         ...
                         <span className="text-[10px] font-bold uppercase text-black/50">ELIMS</span>
                     </div>
                 </div>
             ))}
         </div>
    ...
    return (
      <div className="flex flex-col h-full bg-black animate-in fade-in duration-300 font-sans select-none overflow-hidden rounded-tl-xl border-l border-t border-white/5">
        ...
        <header className="h-16 bg-black border-b border-white/10 flex items-center px-8 justify-between shrink-0 z-[100]">
          <div className="flex items-center gap-12">
              <div className="flex flex-col">
                  <div className="flex items-center gap-1.5">
                      <span className="text-white font-[1000] italic text-lg tracking-tighter uppercase leading-none">SETUP</span>
                      <span className="text-[#ccff00] font-[1000] italic text-lg tracking-tighter uppercase leading-none">ASSET</span>
                  </div>
                  <span className="text-[7px] font-black text-zinc-700 tracking-[0.4em] uppercase mt-1">MASTER CONFIGURATION</span>
              </div>
          ...
          <div className="flex items-center gap-3">
              <div className="bg-zinc-900 border border-white/5 px-3 py-1.5 rounded"><span className="text-[7px] font-black text-zinc-600 uppercase mr-2">EDITING:</span><span className="text-[9px] font-black text-white uppercase">{activeAsset.name}</span></div>
              <button onClick={() => setShowList(!showList)} className={`p-2 rounded border transition-all ${showList ? 'bg-[#ccff00]/10 border-[#ccff00]/40 text-[#ccff00]' : 'bg-zinc-950 border-white/5 text-zinc-500'}`} title="Toggle Sidebar"><LayoutTemplate size={14} /></button>
              <button onClick={() => setShowMonitors(!showMonitors)} className={`p-2 rounded border transition-all ${showMonitors ? 'bg-[#ccff00]/10 border-[#ccff00]/40 text-[#ccff00]' : 'bg-zinc-950 border-white/5 text-zinc-500'}`} title="Toggle Monitors"><Monitor size={14} /></button>
          </div>
        </header>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <aside className={`border-r border-white/5 bg-black flex flex-col shrink-0 transition-all duration-300 ease-in-out ${showList ? 'w-64 opacity-100 p-6' : 'w-0 opacity-0 p-0 pointer-events-none'}`}>
              <h3 className="text-[8px] font-black text-zinc-700 tracking-[0.4em] uppercase mb-6 italic">AVAILABLE TEMPLATES</h3>
              <div className="space-y-3 overflow-y-auto no-scrollbar">
                  ...
                  {gameSpecificAssets.length === 0 && (
                      <div className="py-10 text-center opacity-20 border border-dashed border-white/10 rounded-xl">
                          <span className="text-[8px] font-black uppercase tracking-widest">NO ASSETS FOUND</span>
                      </div>
                  )}
              </div>
          </aside>
                  ...
                  <div className="max-w-2xl mx-auto space-y-8">
                      ...
                      <div className="flex items-center justify-between border-b border-white/5 pb-6">
                          <div className="flex items-center gap-5">
                              ...
                              <div>
                                  <h2 className="text-2xl font-[1000] italic text-white uppercase tracking-tighter leading-none">CUSTOMIZE <span className="text-[#ccff00]">UI</span></h2>
                                  <p className="text-[7px] font-black text-zinc-700 tracking-[0.4em] uppercase mt-1">CONFIGURING: {activeAsset.name}</p>
                              </div>
                          </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="bg-zinc-900/50 border border-white/5 rounded-2xl p-6 space-y-4">
                              <h3 className="text-[9px] font-black text-zinc-500 tracking-[0.2em] uppercase flex items-center gap-2"><Type size={12} /> MAIN TITLE</h3>
                              <div className="space-y-3">
                                  ...
                                  <div className="grid grid-cols-3 gap-2">
                                      <div><label className="text-[7px] font-black text-zinc-700 mb-1 block">SIZE</label><ScrollableInput value={typography.titleSize} onChange={(v) => setTypography({...typography, titleSize: v})} className="w-full bg-black border border-white/10 rounded px-2 py-1.5 text-center text-[10px] font-black text-white" /></div>
                                      <div><label className="text-[7px] font-black text-zinc-700 mb-1 block">X</label><ScrollableInput value={typography.titleX} onChange={(v) => setTypography({...typography, titleX: v})} className="w-full bg-black border border-white/10 rounded px-2 py-1.5 text-center text-[10px] font-black text-white" /></div>
                                      <div><label className="text-[7px] font-black text-zinc-700 mb-1 block">Y</label><ScrollableInput value={typography.titleY} onChange={(v) => setTypography({...typography, titleY: v})} className="w-full bg-black border border-white/10 rounded px-2 py-1.5 text-center text-[10px] font-black text-white" /></div>
                                  </div>
                              </div>
                          </div>
                          <div className="bg-zinc-900/50 border border-white/5 rounded-2xl p-6 space-y-4">
                              <h3 className="text-[9px] font-black text-zinc-500 tracking-[0.2em] uppercase flex items-center gap-2"><ImageIcon size={12} /> GLOBAL SKIN</h3>
                              <div className="grid grid-cols-1 gap-2">
                                  <div className="flex items-center justify-between p-2 bg-black border border-white/5 rounded-lg">
                                      <span className="text-[8px] font-black text-zinc-600 uppercase">PRIMARY</span>
                                      <div className="w-4 h-4 rounded bg-[#ccff00] shadow-[0_0_5px_#ccff00]" />
                                  </div>
                                  <div className="flex items-center justify-between p-2 bg-black border border-white/5 rounded-lg">
                                      <span className="text-[8px] font-black text-zinc-600 uppercase">SECONDARY</span>
                      ...
                      <div className="pt-4 border-t border-white/5">
                          <div className="flex items-center justify-between mb-4">
                              <h3 className="text-[9px] font-black text-zinc-500 tracking-[0.3em] uppercase italic">LIVE STATS DEPLOYMENT</h3>
                              <button className="text-[8px] font-black text-[#ccff00] hover:underline">RANDOMIZE</button>
                          </div>
