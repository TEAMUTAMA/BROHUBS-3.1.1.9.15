...
export function getChildMotionInitial(
  ...
  switch (config.inType) {
    ...
    case 'slide-left':
    case 'leaderboard-slide-left':
      return { x: magnitude, opacity: 0 };
    case 'slide-up':
    case 'leaderboard-slide-up':
...
function getMotionTransition(config: AnimationConfig): any {
  return config.useSpring
    ? {
...
export function getRootMotionProps(config: AnimationConfig) {
  ...
  const transition = getMotionTransition(config);
...
export const getAnimationVariants = (config: AnimationConfig): Variants => {
  const getInitial = (type: string) => {
    switch (type) {
      ...
      case 'slide-left':
      case 'leaderboard-slide-left': return { x: 1920, opacity: 0 };
      case 'slide-up':
      case 'leaderboard-slide-up': return { y: 1080, opacity: 0 };
