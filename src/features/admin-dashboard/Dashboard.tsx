...
import MemberOutputView from '@/features/member-dashboard/MemberOutputView';
import MemberManagement from './MemberManagement';
import GlobalStudio from './GlobalStudio';
import GameView from '@/features/games/game-view/GameView'; 
import AssetView from '@/features/assets/AssetView'; 
import PubgMobileView from '@/features/games/pubg-mobile/PubgMobileView'; 
import MobileLegendsView from '@/features/games/mobile-legends/MobileLegendsView';
import ValorantView from '@/features/games/valorant/ValorantView';
import FreeFireView from '@/features/games/free-fire/FreeFireView';
import HonorOfKingsView from '@/features/games/honor-of-kings/HonorOfKingsView';
import Dota2View from '@/features/games/dota2/Dota2View';
import PricingModal from '@/components/shared/PricingModal';
import MemberProjectTerminal from '@/features/member-dashboard/MemberProjectTerminal'; 
import MemberProjectDetailsView from '@/features/member-dashboard/MemberProjectDetailsView';
import MemberBroadcastView from '@/features/member-dashboard/MemberBroadcastView';
...
import { MemberNotificationModal } from '@/features/notifications';
import { Member, Game, Theme, Asset, AccessTier, Project, PlayerData, Ad, AppUpdateTask, MemberNotification } from '@/types';
...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const [announcementPriority, setAnnouncementPriority] = useState<'INFO' | 'WARNING' | 'IMPORTANT'>('INFO');
  ...
  const handleVerifyAndEnterPOV = () => {
    if (!verificationInput) {
      setVerificationError(t('dr.errEnterCode'));
      return;
    }
    ...
    if (verificationInput.trim() === verificationCode) {
      ...
      setVerificationError(t('dr.errCodeMismatch'));
    }
  };
  ...
  const BHS_LOGO_URL =
    'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 64 64%22%3E%3Crect width=%2264%22 height=%2264%22 rx=%2214%22 fill=%22%23ccff00%22/%3E%3Ctext x=%2232%22 y=%2239%22 text-anchor=%22middle%22 font-family=%22Arial,sans-serif%22 font-size=%2220%22 font-weight=%22900%22 fill=%22%23000%22%3EBHS%3C/text%3E%3C/svg%3E';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [fetchedMembers, fetchedGames, fetchedThemes, fetchedTasks, fetchedNotifications] = await Promise.all([
            getMembers(),
            getGames(),
            getThemes(),
            getTasks(),
  ...
  // Tampilkan modal notifikasi saat member pertama kali masuk dashboard
  useEffect(() => {
    ...
    const hasUnread = notifications.some(
      (n) =>
        !n.readBy.includes(email) &&
        (!n.targetEmails?.length || n.targetEmails.includes(email))
    );
    if (hasUnread) setShowNotifModal(true);
  }, [userRole, isDataLoaded, currentUserEmail, currentMemberEmail, notifications]);
  ...
  const handleMemberPasswordSetup = (verificationCode: string, newPassword: string): string | null => {
    ...
    if (!result.ok) {
      return 'error' in result ? result.error : 'Gagal menyimpan password.';
    }
    ...
    return (
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        <header className="flex flex-col md:flex-row justify-between items-start mb-12 gap-6">
          <div>
            <h1 className="text-4xl md:text-5xl font-black tracking-tighter mb-2 uppercase italic text-white">{t('dr.adManagementTitle')}</h1>
            <p className="text-gray-500 text-sm max-w-lg leading-relaxed font-medium">{t('dr.adManagementSubtitle')}</p>
          </div>
          <button 
            onClick={handleAddAd}
        ...
        <div className="grid grid-cols-1 gap-8">
                ...
                <div className="lg:w-1/2 p-8 flex flex-col justify-between">
                  <div className="space-y-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <span className={`text-[10px] font-black tracking-widest uppercase ${ad.active ? 'text-[#ccff00]' : 'text-zinc-500'}`}>
                          {ad.active ? t('dr.activeNode') : t('dr.inactive')}
                        </span>
                      </div>
                      <button 
                      ...
                      >
                        {ad.active ? t('dr.deactivate') : t('dr.activate')}
                      </button>
                    </div>

                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-gray-500 tracking-widest uppercase">{t('dr.targetUrlLabel')}</label>
                    ...
                    <div className="space-y-3 pt-4 border-t border-white/5">
                      <label className="text-[10px] font-black text-gray-500 tracking-widest uppercase">{t('dr.displayLocation')}</label>
                      <select
                      ...
                      >
                        <option value="LANDING">{t('dr.locLandingSlider')}</option>
                        <option value="LANDING_POPUP">{t('dr.locLandingPopup')}</option>
                        <option value="DASHBOARD">{t('dr.locDashboardPopup')}</option>
                        <option value="PROJECTS">{t('dr.locProjectsPopup')}</option>
                        <option value="GAMES">{t('dr.locGamesPopup')}</option>
                        <option value="ASSETS">{t('dr.locAssetsPopup')}</option>
                      </select>
                    </div>

                    {ad.displayLocation && ad.displayLocation !== 'LANDING' && (
                      <div className="space-y-3 pt-4 border-t border-white/5">
                        <label className="text-[10px] font-black text-gray-500 tracking-widest uppercase">{t('dr.popUpSize')}</label>
                        <select
                          value={ad.popUpSize || 'MEDIUM'}
                          onChange={(e) => handleUpdateAd(ad.id, { popUpSize: e.target.value as any })}
                          className="w-full bg-black border border-white/10 rounded-xl py-4 px-4 text-sm font-medium text-white focus:border-[#ccff00] focus:ring-1 focus:ring-[#ccff00] outline-none transition-all appearance-none"
                        >
                          <option value="SMALL">{t('dr.sizeSmall')}</option>
                          <option value="MEDIUM">{t('dr.sizeMedium')}</option>
                          <option value="LARGE">{t('dr.sizeLarge')}</option>
                          <option value="ASPECT_16_9">{t('dr.size169')}</option>
                        </select>
                      </div>
                    )}

                    <div className="space-y-3 pt-4 border-t border-white/5">
                      <label className="text-[10px] font-black text-gray-500 tracking-widest uppercase">{t('dr.overlayControls')}</label>
                  ...
                  <div className="flex items-center justify-between mt-8 pt-6 border-t border-white/5">
                    <div className="flex items-center gap-2 text-gray-600">
                      <Info size={12} />
                      <span className="text-[9px] font-bold tracking-widest uppercase italic">{t('dr.autoSaved')}</span>
                    </div>
                    <button 
          ...
          {ads.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed border-white/5 rounded-[40px] bg-white/[0.02]">
              ...
              <h3 className="text-xl font-black text-zinc-500 uppercase italic tracking-tight mb-2">{t('dr.noActiveAds')}</h3>
              <p className="text-[10px] font-bold text-zinc-600 tracking-[0.2em] uppercase mb-8 text-center max-w-xs">{t('dr.noActiveAdsDesc')}</p>
              <button 
                onClick={handleAddAd}
  ...
  const allNavItems = [
    { icon: LayoutDashboard, label: 'OVERVIEW', tkey: 'dash.nav.overview', roles: ['admin'] },
    { icon: Layers, label: 'PROJECTS', tkey: 'dash.nav.projects', roles: ['admin', 'member'] },
    { icon: LayoutTemplate, label: 'STUDIO', tkey: 'dash.nav.studio', roles: ['admin'] },
    { icon: Radio, label: 'BROADCAST', tkey: 'dash.nav.broadcast', roles: ['admin', 'member'] },
    { icon: Share2, label: 'OUTPUT', tkey: 'dash.nav.output', roles: ['admin', 'member'] },
    { icon: Sparkles, label: 'UPDATES', tkey: 'dash.nav.updates', roles: ['admin', 'member'] },
    { icon: SettingsIcon, label: 'SETTINGS', tkey: 'dash.nav.settings', roles: ['admin'] },
    { icon: Users, label: 'MEMBER', tkey: 'dash.nav.member', roles: ['admin'], notification: resetRequests.length > 0 || pendingExtensionCount > 0 },
  ];
  ...
  const toggleGameRelease = (gameId: string) => {
    ...
    const releasing = !game?.isReleased;
    setGames((prev) => prev.map((g) => (g.id === gameId ? { ...g, isReleased: !g.isReleased } : g)));
    if (releasing && game) {
      const notif: MemberNotification = {
        id: `game-${gameId}-${Date.now()}`,
        type: 'GAME_ASSET',
        title: `Game Baru Tersedia: ${game.title}`,
        message: `${game.title} kini sudah tersedia di platform. Buka menu GAMES untuk mulai membuat project overlay-mu!`,
        assetType: 'game',
  ...
  const handleBroadcastTask = (task: AppUpdateTask) => {
    const existing = notifications.find((n) => n.taskId === task.id);
    if (existing) {
      // Reset readBy agar semua member dapat lagi
      setNotifications((prev) =>
        prev.map((n) => (n.id === existing.id ? { ...n, readBy: [], createdAt: new Date().toISOString() } : n))
      );
      return;
    }
    const notif: MemberNotification = {
      ...
      createdAt: new Date().toISOString(),
      readBy: [],
      priority: task.priority === 'CRITICAL' ? 'IMPORTANT' : task.priority === 'HIGH' ? 'WARNING' : 'INFO',
    };
    setNotifications((prev) => [notif, ...prev]);
  };
  ...
  const handleAcknowledgeNotifications = (ids: string[]) => {
    ...
    setNotifications((prev) =>
      prev.map((n) =>
        ids.includes(n.id) && !n.readBy.includes(email)
          ? { ...n, readBy: [...n.readBy, email] }
          : n
      )
    );
    ...
    return (
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        <header className="mb-8 flex flex-col md:flex-row justify-between md:items-end border-b border-white/5 pb-6 gap-6">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-white mb-2 uppercase italic">{t('dr.systemSettingsTitle')}</h1>
            <p className="text-gray-500 text-sm font-medium">{t('dr.systemSettingsSubtitle')}</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-zinc-900 border border-white/5 rounded-[32px] p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="text-lg font-black tracking-tight text-white uppercase italic">{t('dr.globalBrandingTitle')}</h3>
                  <p className="text-[10px] font-bold text-gray-500 tracking-widest uppercase">{t('dr.globalBrandingDesc')}</p>
                </div>
              </div>
                ...
                <input 
                  ...
                  onChange={(e) => {
                    ...
                    if (file && onGlobalUpload) {
                      void compressImage(file, LOGO_PRESET).then((dataUrl) => onGlobalUpload(dataUrl));
                    }
                  }}
                ...
                `}>
                  {globalLogo ? (
                    ...
                    <>
                      ...
                      <p className="text-gray-500 font-bold text-xs tracking-widest uppercase text-center">{t('dr.uploadBrohubsLogo')}</p>
                    </>
                  )}
                </div>
              </div>
              ...
              >
                <RotateCcw size={14} /> {t('dr.resetGlobalLogo')}
              </button>
            </div>
            ...
            <div className="bg-zinc-900 border border-white/5 rounded-[32px] p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="text-lg font-black tracking-tight text-white uppercase italic">{t('dr.adminHubTitle')}</h3>
                  <p className="text-[10px] font-bold text-gray-500 tracking-widest uppercase">{t('dr.adminHubDesc')}</p>
                </div>
              </div>
                ...
                <input 
                  ...
                  onChange={(e) => {
                    ...
                    if (file && onAdminUpload) {
                      void compressImage(file, LOGO_PRESET).then((dataUrl) => onAdminUpload(dataUrl));
                    }
                  }}
                  className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                />
                ...
                `}>
                  {adminLogo ? (
                    ...
                    <>
                      ...
                      <p className="text-[#ccff00] font-bold text-xs tracking-widest uppercase text-center">{t('dr.uploadBhsLogo')}</p>
                    </>
                  )}
                </div>
              </div>
              ...
              >
                <RotateCcw size={14} /> {t('dr.resetAdminLogo')}
              </button>
            </div>
          </div>
        ) : (
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'UPDATES') {
      return (
        <div className="space-y-6">
          {/* Admin: Panel Pengumuman + Notifikasi */}
          {userRole === 'admin' && (
            <div className="bg-zinc-950 border border-white/10 rounded-2xl p-6">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h3 className="text-sm font-black text-white uppercase tracking-widest">Kirim Pengumuman</h3>
                  <p className="text-[10px] text-zinc-500 mt-0.5">Pesan ini akan muncul sebagai popup saat member buka dashboard</p>
                </div>
                {notifications.length > 0 && (
                  <div className="ml-auto flex items-center gap-2 text-[10px] text-zinc-500">
                    <span>{notifications.length} notifikasi aktif</span>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                <div className="md:col-span-2">
                  <input
                    ...
                    className="w-full bg-zinc-900 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-yellow-500/40"
                  />
                </div>
                <select
                  value={announcementPriority}
                  onChange={(e) => setAnnouncementPriority(e.target.value as 'INFO' | 'WARNING' | 'IMPORTANT')}
                  className="bg-zinc-900 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-yellow-500/40"
                >
              ...
              <div className="flex gap-3">
                <textarea
                  ...
                  placeholder="Isi pesan pengumuman untuk semua member..."
                  rows={2}
                  className="flex-1 bg-zinc-900 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-yellow-500/40 resize-none"
                />
                <button
                  onClick={handleCreateAnnouncement}
                  disabled={!announcementTitle.trim() || !announcementMsg.trim()}
                  className={`flex-shrink-0 flex flex-col items-center justify-center gap-1 px-5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all disabled:opacity-30
                    ${announcementSent
                      ? 'bg-green-500/20 border border-green-500/30 text-green-400'
                      : 'bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 hover:bg-yellow-500 hover:text-black'
                    }`}
                >
                  {announcementSent ? <Check size={16} /> : <Megaphone size={16} />}
                  {announcementSent ? 'Terkirim!' : 'Kirim'}
                </button>
              </div>

              {/* Tombol broadcast di task: info hint */}
              <p className="text-[10px] text-zinc-600 mt-3">
                💡 Klik ikon <span className="text-yellow-400">📣</span> pada task di bawah untuk broadcast update task ke semua member.
              </p>
            </div>
          )}
          ...
          <TasksManagement
            ...
            onBroadcastTask={userRole === 'admin' ? handleBroadcastTask : undefined}
          />
        </div>
      );
    }
        ...
        return (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="mb-8 flex flex-col md:flex-row justify-between md:items-end gap-4">
                    <div>
                        <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-white mb-2">{t('dr.masterAssetHubTitle')}</h1>
                        <p className="text-gray-500 text-sm font-medium">{t('dr.masterAssetHubSubtitle')}</p>
                    </div>
                </div>
                
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-12">
                     <button className="bg-[#ccff00] text-black px-6 py-2.5 rounded-xl font-black text-[10px] tracking-[0.2em] uppercase hover:scale-105 transition-all shadow-[0_0_15px_rgba(204,255,0,0.3)] border border-[#ccff00]">{t('dr.allGames')}</button>
                    <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#ccff00]/10 border border-[#ccff00]/20 text-[#ccff00]">
                        <Info size={14} />
                        <span className="text-[10px] font-black tracking-widest uppercase">{t('dr.serverUplinkStable')}</span>
                    </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {/* Urutkan: game yang sudah Ready/Released (isReleased) tampil lebih dulu; sisanya tetap urutannya */}
                    {[...games]
                        .sort((a, b) => (b.isReleased ? 1 : 0) - (a.isReleased ? 1 : 0))
                        .map((game) => {
                        ...
                        >
                             ...
                             <div className="absolute bottom-0 left-0 p-8 w-full z-10">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    {!game.isReleased && (<span className="bg-red-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded tracking-tighter">{t('dr.unreleased')}</span>)}
                                </div>
          ...
          <>
            <header className="flex flex-col md:flex-row justify-between items-start mb-16 gap-6">
              <div className="animate-in slide-in-from-top-4 duration-500">
                <h1 className="text-4xl md:text-5xl font-black tracking-tighter mb-2 uppercase italic text-white">{t('dash.overviewTitle')}</h1>
                <p className="text-gray-500 text-sm max-w-lg leading-relaxed font-medium">{t('dash.overviewSubtitle')}</p>
              </div>
              <div className="flex items-center gap-3 px-4 py-2 rounded-full border border-[#ccff00]/20 bg-[#ccff00]/5 self-start shadow-[0_0_20px_rgba(204,255,0,0.05)]">
                ...
                <span className="text-[10px] font-black tracking-[0.2em] text-[#ccff00] uppercase">{t('dash.systemNominal')}</span>
              </div>
            </header>
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
              <div className="p-8 rounded-[24px] bg-zinc-900 border border-white/5 hover:border-white/10 transition-all group relative overflow-hidden">
                <div className="flex justify-between items-start mb-8 z-10 relative">
                  <p className="text-[10px] font-black text-gray-500 tracking-[0.2em] uppercase">{t('dash.totalHubMembers')}</p>
                ...
                <div className="flex flex-col gap-1 mb-6 z-10 relative">
                  ...
                  <span className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">{t('dash.registeredAccounts')}</span>
                </div>
              ...
              <div className="p-8 rounded-[24px] bg-zinc-900 border border-white/5 hover:border-[#ccff00]/30 transition-all group relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-8 z-10 relative">
                  <p className="text-[10px] font-black text-gray-500 tracking-[0.2em] uppercase">{t('dash.activeBroadcasters')}</p>
                ...
                <div className="flex flex-col gap-1 mb-6 z-10 relative">
                  ...
                  <span className="text-[10px] font-bold text-[#ccff00]/60 uppercase tracking-widest">{t('dash.currentlyInStream')}</span>
                </div>
              ...
              <div className="p-8 rounded-[24px] bg-zinc-900 border border-white/5 hover:border-blue-500/30 transition-all group relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-8 z-10 relative">
                  <p className="text-[10px] font-black text-gray-500 tracking-[0.2em] uppercase">{t('dash.onlinePresence')}</p>
                ...
                <div className="flex flex-col gap-1 mb-6 z-10 relative">
                  ...
                  <span className="text-[10px] font-bold text-blue-500/60 uppercase tracking-widest">{t('dash.idleInDashboard')}</span>
                </div>
                <div className="w-full h-1.5 bg-black rounded-full overflow-hidden z-10 relative">
            ...
            <section className="animate-in slide-in-from-bottom-4 duration-700">
               <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
                <h2 className="text-[11px] font-black text-gray-500 tracking-[0.2em] uppercase">{t('dash.memberNetworkHealth')}</h2>
              </div>
                  ...
                  <div key={idx} className="p-5 rounded-[20px] bg-black border border-white/5 hover:border-white/10 transition-all flex items-center justify-between group cursor-default">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-black text-gray-600 tracking-wider uppercase">{member.package} {t('dash.memberSuffix')}</p>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <p className={`text-lg font-black ${member.status === 'IN STREAM' ? 'text-white' : (member.status === 'ONLINE' ? 'text-blue-500' : 'text-zinc-600')}`}>{member.status === 'OFFLINE' ? t('dash.statusOffline') : t('dash.statusStable')}</p>
                    </div>
                  </div>
                ))}
              </div>
    ...
    <div className="flex h-screen bg-black text-white font-inter overflow-hidden relative flex-col md:flex-row">
      ...
      `}>
        <div className="hidden md:flex flex-col items-center mb-10 w-full">
          <button onClick={() => setIsExpanded(!isExpanded)} className={`mb-4 flex items-center justify-center rounded-xl border border-white/5 bg-zinc-900/50 hover:bg-zinc-900 transition-all group ${isExpanded ? 'w-full py-3 px-4 justify-between' : 'w-12 h-12'}`}>{isExpanded ? (<><span className="text-[10px] font-black tracking-[0.2em] text-gray-500 uppercase">{t('dash.hideMenu')}</span><X size={16} className="text-[#ccff00]" /></>) : (<Menu size={20} className="text-gray-400 group-hover:text-white" />)}</button>
          {!isExpanded && (<div className="animate-in fade-in zoom-in duration-500 flex flex-col items-center"><div className="w-12 h-12 flex items-center justify-center bg-[#ccff00] rounded-2xl shadow-[0_0_20px_rgba(204,255,0,0.2)] transition-all duration-300 overflow-hidden mb-3"><img src={globalLogo || BHS_LOGO_URL} alt="BHS Logo" className="w-full h-full object-contain p-2.5" /></div><div className="w-1 h-1 bg-[#ccff00] rounded-full animate-pulse" /></div>)}
          {isExpanded && (<div className="w-full px-2 mt-2 animate-in fade-in slide-in-from-top-4 duration-500"><div className="p-4 rounded-2xl bg-zinc-900/40 border border-white/5 shadow-inner"><p className="text-[9px] font-black text-gray-600 tracking-[0.4em] uppercase mb-2 flex items-center gap-2">{t('dash.systemNode')}</p><h3 className="text-sm font-[1000] italic tracking-tighter uppercase text-white leading-none mb-4">BRO<span className="text-[#ccff00]">HUBS</span> {userRole === 'admin' ? 'ADMIN' : 'STUDIO'}</h3><div onClick={() => userRole === 'member' && setIsPackageModalOpen(true)} className={`flex items-center gap-2 group/meta ${userRole === 'member' ? 'cursor-pointer hover:opacity-80' : ''}`}><div className="w-1.5 h-1.5 rounded-full bg-[#ccff00] shadow-[0_0_8px_#ccff00] animate-pulse" /><span className="text-[8px] font-black text-[#ccff00] tracking-[0.2em] uppercase">{getSidebarPlanLabel()}</span>{userRole === 'member' && <ChevronRight size={10} className="text-[#ccff00] opacity-0 group-hover/meta:opacity-100 transition-opacity" />}</div></div></div>)}
        </div>
        ...
        <div className="hidden md:flex mt-auto space-y-4 w-full flex-col items-center">
          {userRole === 'member' && isExpanded && getActiveMember() && !isPasswordlessMember(getActiveMember()!) && (
            ...
            >
              <Key size={12} /> {t('member.changePassword')}
            </button>
          )}
          <LanguageSwitcher isExpanded={isExpanded} />
          <div className={`rounded-2xl bg-zinc-900/50 border border-white/5 flex flex-col transition-all overflow-hidden ${isExpanded ? 'p-4 w-full' : 'p-3 w-14 items-center justify-center'}`}>{isExpanded ? (<><div className="flex items-center gap-2 mb-1"><Clock size={10} className="text-gray-500" /><p className="text-[8px] font-bold text-gray-600 tracking-widest uppercase">{t('dash.uptime')}</p></div><p className="text-[#ccff00] font-mono text-[10px]">{uptime}</p></>) : (<Clock size={20} className="text-[#ccff00] animate-pulse" />)}</div><button onClick={onExit} className={`border border-white/10 rounded-2xl font-bold tracking-[0.2em] uppercase hover:bg-red-500/10 hover:border-red-500/30 hover:text-red-500 transition-all active:scale-95 flex items-center justify-center ${isExpanded ? 'w-full py-4 text-[10px]' : 'w-14 h-14'}`}>{isExpanded ? (<div className="flex items-center gap-2"><LogOut size={14} /><span>{t('common.exit')}</span></div>) : (<LogOut size={20} />)}</button></div>
      </aside>
      ...
      {subModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            `}>
                ...
                <h2 className="text-2xl font-[1000] tracking-tight uppercase text-white mb-2 leading-none">{subModal === 'EXPIRED' ? t('dr.accessExpired') : t('dr.renewalNotice')}</h2>
                <p className="text-[10px] font-black text-zinc-500 tracking-[0.2em] uppercase mb-10">{subModal === 'EXPIRED' ? t('dr.uplinkTerminated') : t('dr.expiresInDays').replace('{days}', String(daysRemaining))}</p>
                <div className="flex flex-col gap-4 w-full">
                    {subModal === 'EXPIRED' ? (
                         <button onClick={() => handleSubAction('EXTEND')} className="w-full py-5 rounded-2xl bg-[#ccff00] text-black font-[1000] text-sm tracking-widest transition-all uppercase hover:scale-[1.02] shadow-[0_0_30px_rgba(204,255,0,0.4)]">{t('dr.renewAccess')}</button>
                    ) : (
                        <>
                            <button onClick={() => handleSubAction('EXTEND')} className={`w-full py-5 rounded-2xl font-black text-[10px] tracking-widest transition-all uppercase hover:scale-[1.02] ${daysRemaining <= 3 ? 'bg-yellow-500 text-black shadow-[0_0_20px_rgba(245,158,11,0.3)]' : 'bg-white text-black'}`}>{t('dr.extendPlan')}</button>
                             <button onClick={() => handleSubAction('CONTINUE')} className="w-full py-5 rounded-2xl bg-zinc-900 border border-white/5 text-[10px] font-black tracking-widest text-gray-400 hover:text-white uppercase">{t('dr.continueLater')}</button>
                        </>
                    )}
                </div>
            </div>
      ...
      {pendingReset && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-zinc-950 border border-amber-500/20 rounded-[32px] overflow-hidden p-8 animate-in zoom-in-95 shadow-2xl flex flex-col items-center text-center">
            ...
            <h2 className="text-2xl font-black tracking-wide uppercase text-white mb-4">{t('dr.securityRequest')}</h2>
            <p className="text-[10px] font-black text-zinc-500 tracking-[0.2em] uppercase leading-relaxed max-w-[280px] mb-8">{t('dr.securityRequestDesc')}</p>
            <div className="grid grid-cols-2 gap-4 w-full">
                <button onClick={handleMemberDenyReset} className="py-4 rounded-xl bg-zinc-900 border border-white/10 text-zinc-400 hover:text-white transition-all text-[10px] font-black tracking-widest uppercase">{t('dr.cancelDeny')}</button>
                <button onClick={handleMemberAcceptReset} className="py-4 rounded-xl bg-amber-500 text-black border border-amber-500 hover:bg-amber-400 transition-all text-[10px] font-black tracking-widest uppercase shadow-[0_0_20px_rgba(245,158,11,0.3)]">{t('dr.resetAccept')}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showMemberSuccessModal && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-[380px] bg-zinc-950 border border-white/10 rounded-[32px] overflow-hidden p-8 animate-in zoom-in-95 shadow-2xl flex flex-col items-center text-center">
            ...
            <h2 className="text-xl font-black tracking-wide uppercase text-white mb-2">{t('dr.accessGranted')}</h2>
            <p className="text-[10px] font-black text-gray-500 tracking-[0.2em] uppercase mb-6">{t('dr.newSystemGeneratedKey')}</p>
      ...
      {previewAsset && themes.length > 0 && (
        <AssetPreviewModal
            ...
            userRole={userRole}
            globalLogo={globalLogo}
            projectPlayers={activeProject?.players ?? []}
            companionProjectScope={activeProject?.id ?? null}
        />
      )}
          ...
          <div className="relative w-full max-w-sm bg-[#080808] border border-white/10 hover:border-[#ccff00]/20 transition-all rounded-[32px] overflow-hidden p-8 animate-in zoom-in-95 shadow-2xl flex flex-col items-center">
            ...
            <div className="w-full bg-zinc-905/40 border border-white/5 rounded-2xl p-4 mb-6 text-left relative overflow-hidden bg-black/40">
              ...
              <span className="text-[8px] font-black text-[#ccff00]/85 tracking-widest uppercase block mb-1">
                📩 {t('dr.smsSentTo').replace('{name}', verificationMember.name.toUpperCase())}
              </span>
              <p className="text-[10px] text-zinc-400 font-medium leading-relaxed italic">
                "{verificationMember.name}, {t('dr.smsBodyPrefix')} <strong className="text-white not-italic font-extrabold bg-[#ccff00]/10 px-1.5 py-0.5 rounded font-mono text-xs">{verificationCode}</strong> {t('dr.smsBodySuffix')}"
              </p>
              ...
              >
                <Zap size={10} className="fill-current" /> {t('dr.pasteCodeAuto')}
              </button>
            </div>
            ...
            <div className="w-full space-y-4">
              <div className="relative">
                <input
                  type="text"
                  maxLength={4}
                  placeholder={t('dr.codePlaceholder')}
      ...
      {/* Notifikasi member: muncul saat masuk dashboard jika ada yang belum dibaca */}
      {showNotifModal && userRole === 'member' && (
        <MemberNotificationModal
          notifications={notifications}
          memberEmail={currentUserEmail || currentMemberEmail || ''}
