    ...
    >
        ...
        {isLocked && userRole === 'member' && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/20">
             ...
             <span className="text-[10px] font-black text-red-500 tracking-[0.3em] uppercase">{t('ac.upgradeToUnlock')}</span>
          </div>
        )}
                        ...
                        {isAdminToolOpen && (
                          <div className="absolute top-0 right-10 h-8 bg-zinc-900 border border-white/10 rounded-full px-4 flex items-center gap-3 animate-in slide-in-from-right-4 duration-300 shadow-2xl whitespace-nowrap">
                             ...
                             >
                                ...
                                {asset.isReleased ? t('ac.live') : t('ac.hidden')}
                             </button>
                             <div className="w-px h-3 bg-white/10" />
                             <button 
        ...
        <div className="relative z-10 flex flex-col flex-1">
          <div className="flex items-center gap-2 mb-3">
            ...
            {userRole === 'admin' && !asset.isReleased && (
              <span className="bg-red-500/20 border border-red-500/30 text-red-500 text-[7px] font-black px-1.5 py-0.5 rounded uppercase tracking-tighter">{t('ac.hiddenBadge')}</span>
            )}
          </div>
            ...
            {userRole === 'admin' ? (
                ...
                <>
                    ...
                    >
                        ...
                        {isAlreadyInProject ? t('ac.alreadyInProject') : isLocked ? t('ac.upgradeToAdd') : t('ac.addAssetToProject')}
                    </button>
                </>
            )}
          </div>
