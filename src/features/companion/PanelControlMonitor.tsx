      ...
      >
        ...
        <div className="p-6 pb-2 flex items-center justify-between relative z-10 shrink-0">
          ...
          >
            ...
            {isLocked ? t('pcm.locked') : t('pcm.unlocked')}
          </button>
        </div>
            ...
            <div className="flex items-start justify-between min-h-8">
              ...
              <div className="flex flex-col items-end shrink-0 gap-2">
                <div className="flex items-baseline gap-3">
                  ...
                  <button 
                    onClick={() => window.open(programOutputUrl || `${window.location.origin}${window.location.pathname}?view=program`, '_blank')} 
                  ...
                  >
                    ...
                    {isProgramHidden ? t('pcm.show') : t('pcm.hide')}
                  </button>
              ...
              >
                <div className="w-full h-full relative">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center opacity-10 pointer-events-none">
                     ...
                     <span className="text-[10px] font-black tracking-[1em] italic text-white uppercase">{t('pcm.signalLost')}</span>
                  </div>
                    ...
                    >
                        ...
                        >
                          <AnimatePresence mode="wait" initial={false}>
                            {programPreview ? (
                              React.isValidElement(programPreview) ? (
                                React.cloneElement(programPreview, {
            ...
            <div className="flex items-start justify-between min-h-8">
              <div className="flex flex-col flex-1">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-[10px] font-black text-white tracking-widest uppercase italic leading-none">{t('pcm.previewStaging')}</span>
                </div>
                <span className="text-[7px] font-black text-zinc-700 tracking-[0.3em] uppercase mt-2 ml-5">{t('pcm.operatorMaster')}</span>
              </div>
              ...
              <div className="flex items-center gap-1">
                    ...
                    {isMultiStopDropdownOpen && (
                      ...
                      >
                        <h4 className="text-[8px] font-black tracking-widest text-[#ccff00] uppercase italic mb-3 flex items-center gap-1.5 pb-2 border-b border-white/5">
                          <Trash2 size={10} /> {t('pcm.stopMultipleAssets')}
                        </h4>

                        {activeAssetsData.length === 0 ? (
                          <p className="text-[9px] text-zinc-500 italic py-4 text-center">{t('pcm.noActiveTemplates')}</p>
                        ) : (
                          <>
                                  ...
                                  >
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <p className="text-[6px] text-zinc-500 font-bold uppercase tracking-widest mt-1">
                                        {a.isProgram
                                          ? t('pcm.programLayer').replace('{layer}', a.layer !== undefined ? String(a.layer) : '')
                                          : t('pcm.previewFeed')
                                        }
                                      </p>
                                    </div>
                            ...
                            <div className="flex gap-2">
                              ...
                              >
                                {t('pcm.stopCount').replace('{count}', String(selectedIds.length))}
                              </button>
                            </div>
                          </>
                        )}
                ...
                {onPlayAction && !isMonitorExpanded && (
                  <button 
                    ...
                    title={playStatus === 2 ? t('pcm.stopProgram') : playStatus === 1 ? t('pcm.stopPreview') : t('pcm.playToProgram')}
                  >
        ...
        <div className="p-8 border-t border-white/5 bg-black mt-auto shrink-0 relative z-20">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[8px] font-black text-zinc-700 tracking-[0.3em] uppercase">{t('pcm.networkTelemetry')}</span>
            </div>
            <span className="text-[#00ffd5] text-[9px] font-black tracking-widest uppercase">{t('pcm.upMbps').replace('{value}', String(telemetry.up))}</span>
          </div>
          
          <div className="space-y-2">
             <div className="flex items-center justify-between">
                <span className="text-[8px] font-black text-zinc-800 tracking-[0.3em] uppercase">{t('pcm.nodeStatus')}</span>
                <span className="text-[#ccff00] text-[8px] font-black tracking-widest uppercase">{t('pcm.syncStable')}</span>
             </div>
      ...
      {isMonitorExpanded && (
        <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-xl flex items-center justify-center animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-3 opacity-60">
                ...
                <span className="text-[10px] font-black text-white tracking-[0.5em] uppercase">{t('pcm.fullscreenPreviewMode')}</span>
            </div>
        </div>
      )}
