...
export function getPlayerStatus(team: Pick<LeaderboardTeamState, 'status'>, playerIndex: number): number {
  return team.status[playerIndex] ?? 1;
}

export function getTeamSlotCount(team: Pick<LeaderboardTeamState, 'status' | 'playerKills' | 'playerFinishCredit' | 'playerKnockCredit' | 'playerNames'>): number {
  if (team.playerNames?.length) return team.playerNames.length;

  return Math.max(
    1,
    team.status.length,
    team.playerKills.length,
    team.playerFinishCredit?.length ?? 0,
    team.playerKnockCredit?.length ?? 0
  );
}

export function createTeamStatus(team: Pick<LeaderboardTeamState, 'status' | 'playerKills' | 'playerFinishCredit' | 'playerKnockCredit' | 'playerNames'>, status: number): number[] {
  return Array.from({ length: getTeamSlotCount(team) }, () => status);
}

export function createZeroKills(team: Pick<LeaderboardTeamState, 'status' | 'playerKills' | 'playerFinishCredit' | 'playerKnockCredit' | 'playerNames'>): number[] {
  return Array.from({ length: getTeamSlotCount(team) }, () => 0);
}

export function createEmptyCredits(team: Pick<LeaderboardTeamState, 'status' | 'playerKills' | 'playerFinishCredit' | 'playerKnockCredit' | 'playerNames'>): Array<FinishCredit | null> {
  return Array.from({ length: getTeamSlotCount(team) }, () => null);
}

function normalizeNumberSlots(values: number[] | undefined, size: number, fallback: number): number[] {
  return Array.from({ length: size }, (_, index) => values?.[index] ?? fallback);
}

function normalizeCreditSlots(
  values: Array<FinishCredit | null> | undefined,
  size: number
): Array<FinishCredit | null> {
...
export function cloneLeaderboardTeams<T extends LeaderboardTeamState>(teams: T[]): T[] {
  return teams.map((t) => ({
    ...t,
    status: normalizeNumberSlots(t.status, getTeamSlotCount(t), 1),
    playerKills: normalizeNumberSlots(t.playerKills, getTeamSlotCount(t), 0),
    playerFinishCredit: normalizeCreditSlots(t.playerFinishCredit, getTeamSlotCount(t)),
    playerKnockCredit: normalizeCreditSlots(t.playerKnockCredit, getTeamSlotCount(t)),
  }));
}

function ensureFinishCredit(team: LeaderboardTeamState): Array<FinishCredit | null> {
  return normalizeCreditSlots(team.playerFinishCredit, getTeamSlotCount(team));
}

function ensureKnockCredit(team: LeaderboardTeamState): Array<FinishCredit | null> {
  return normalizeCreditSlots(team.playerKnockCredit, getTeamSlotCount(team));
}
...
export function assignPlacementOnTeamElim<T extends LeaderboardTeamState>(
  ...
  const placementRank = activeCount - assignedRanks.length;
  const next = teams.map((t, idx) =>
    idx === teamIndex
      ? { ...t, placementRank, status: createTeamStatus(t, 0) }
      : t
  );
...
export function killFeedEventAffectsTeam(event: KillFeedEvent, teamIndex: number): boolean {
  ...
  return (
    'cascadeKills' in event &&
    (event.cascadeKills ?? []).some((ck) => ck.victim.teamIndex === teamIndex)
  );
}
