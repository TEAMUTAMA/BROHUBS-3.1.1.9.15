  ...
  return roster.map((team) => ({
    ...
    players: team.players.map((entry) => {
      ...
      return {
        ...
        role: coerced.role || db?.role || undefined,
      };
    }),
  }));
}
...
export function buildRosterTeamsFromProject(
  ...
  for (const p of projectPlayers) {
    ...
    const entry: RosterPlayerEntry = { name: playerName, image: p.image, role: p.role };
  ...
  // Pertahankan urutan input (urutan kemunculan tim di Project DB), bukan alfabet.
  // Map menjaga urutan penyisipan = urutan pemain saat awal Project Event dibuat.
  return normalizeRosterTeams(Array.from(map.values()));
}
  ...
  for (const team of teams) {
    ...
    const entries: RosterPlayerEntry[] = team.playerNames
      ...
      .map((name) => {
        ...
        return { name, image: db?.image, role: db?.role };
      });
...
/**
 * Urutan tampilan & paginasi — ikut slot Overall Ranking.
 * Tim tanpa rank ditaruh di akhir dengan urutan INPUT (urutan Project DB) dipertahankan,
 * bukan alfabet. Array.sort stabil (ES2019+), jadi tie-breaker 0 menjaga urutan masuk.
 */
export function sortRosterTeamsByLeaderboardRank(teams: RosterTeam[]): RosterTeam[] {
  return [...teams].sort((a, b) => {
    ...
    return 0; // pertahankan urutan input (Project DB) saat rank seri/kosong
  });
}
