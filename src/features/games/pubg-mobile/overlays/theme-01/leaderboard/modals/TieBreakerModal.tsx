...
import type { MatchKillRules } from '@/features/games/pubg-mobile/logic/leaderboardKillLogic';
import { DEFAULT_MATCH_KILL_RULES } from '@/features/games/pubg-mobile/logic/leaderboardKillLogic';
...
const CRITERIA_LABELS: Record<TieBreakerCriteria, { label: string; descKey: string }> = {
  TOTAL_POINTS: { label: 'TOTAL POINTS', descKey: 'pmod.critTotalPointsDesc' },
  TOTAL_WWCD: { label: 'TOTAL WWCD', descKey: 'pmod.critTotalWwcdDesc' },
  TOTAL_PLACEMENT: { label: 'PLACEMENT POINTS', descKey: 'pmod.critTotalPlacementDesc' },
  ALIVE_PLAYERS: { label: 'ALIVE PLAYERS', descKey: 'pmod.critAlivePLayersDesc' },
  MATCH_KILLS: { label: 'MATCH KILLS', descKey: 'pmod.critMatchKillsDesc' },
  SLOT_RANK: { label: 'SLOT RANK', descKey: 'pmod.critSlotRankDesc' },
};
      ...
      <div className="relative w-full max-w-lg bg-[#0c0c0c] border border-white/10 rounded-[40px] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 max-h-[90vh]">
        ...
        <div className="p-8 border-b border-white/5 bg-[#111] flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.3em]">{t('pmod.matchConfig').replace('{n}', String(currentMatch))}</p>
            </div>
          </div>
        ...
        <div className="p-6 bg-[#0a0a0a] border-b border-white/5 shrink-0">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-[10px] font-black text-white uppercase tracking-widest">{t('pmod.matchRulesHeading')}</h3>
          </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[8px] font-medium text-zinc-600 uppercase tracking-tighter truncate">
                  {t(CRITERIA_LABELS[criteria].descKey)}
                </p>
              </div>
        ...
        <div className="p-8 bg-[#111] border-t border-white/5 flex flex-col gap-4 shrink-0">
          <div className="flex items-start gap-3 p-4 bg-blue-500/5 border border-blue-500/10 rounded-2xl">
            ...
            <p className="text-[9px] font-medium text-blue-400/70 uppercase leading-relaxed tracking-wide">
              {isCustom
                ? t('pmod.tbCustomModeInfo')
                : t('pmod.tbDefaultModeInfo')}
            </p>
          </div>
          <div className="flex gap-4">
            <button onClick={onClose} className="flex-1 py-4 rounded-2xl bg-zinc-900 border border-white/5 text-[10px] font-black text-zinc-500 uppercase tracking-widest hover:text-white">{t('pmod.cancel')}</button>
            <button
