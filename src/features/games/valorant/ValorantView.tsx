    ...
    >
        ...
        {!isGlobalLocked && userRole === 'admin' && (
            <div className="absolute right-4 top-4 px-2.5 py-1.5 rounded-lg bg-black/95 border border-white/60 backdrop-blur-sm pointer-events-none whitespace-nowrap z-20 shadow-xl opacity-0 group-hover:opacity-100 transition-opacity">
                <p className="text-[6px] font-black text-[#FF0000] uppercase tracking-widest leading-none mb-0.5">{t('pmv.recSize')}</p>
                <p className="text-[7px] font-bold text-[#FF0000] tracking-wider leading-none">{t('pmv.recSizeValue')}</p>
            </div>
        )}
        ...
        {isGlobalLocked && (
            <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/60 backdrop-blur-[2px] pointer-events-none">
                ...
                <span className="text-[10px] font-black text-gray-400 tracking-[0.2em] uppercase">{t('pmv.comingSoon')}</span>
            </div>
        )}
        ...
        {!isGlobalLocked && isTierLocked && (
            <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/70 backdrop-blur-[1px] pointer-events-none">
                ...
                <span className="text-[10px] font-black text-red-500 tracking-[0.2em] uppercase mb-1">{t('pmv.locked')}</span>
            ...
            <>
                ...
                <div className="absolute top-20 right-4 z-40 flex flex-col gap-2 animate-in fade-in slide-in-from-right-4 duration-300" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button
                        ...
                        title={t('pmv.resetToDefaultImage')}
                    >
        ...
        <div className={`absolute bottom-0 left-0 w-full p-6 z-10 transition-opacity ${isLockedVisual && userRole === 'member' ? 'opacity-20' : 'opacity-100'}`}>
            ...
            <div className="flex items-center justify-between border-t border-white/10 pt-4 cursor-pointer" onClick={handleActionClick}>
                <div>
                    <p className="text-[9px] font-black text-white uppercase tracking-wider mb-0.5">{userRole === 'admin' ? t('pmv.adminControls') : t('pmv.configure')}</p>
                    <p className="text-[7px] font-bold text-gray-600 uppercase tracking-[0.2em]">{t('pmv.fullyCustomizable')}</p>
                </div>
                {userRole === 'admin' && (
...
const TAB_LABELS: Record<string, 'pmv.tabTemplates' | 'pmv.tabWidgets' | 'pmv.tabConfig'> = {
  'TEMPLATES': 'pmv.tabTemplates',
  'WIDGETS': 'pmv.tabWidgets',
  'CONFIG': 'pmv.tabConfig',
};

const ValorantView: React.FC<ValorantViewProps> = ({
  ...
  const valAssets = ASSET_DATABASE.filter(a => a.gameId === 'val');
  ...
  if (selectedTheme && selectedAsset) {
      ...
      if (selectedAsset.id === 'val-team-roster') {
          return (
      ...
      return (
        <div className="flex flex-col h-full bg-black text-white items-center justify-center p-8 animate-in fade-in">
            <h3 className="text-xl font-black mb-2 text-[#ccff00] uppercase tracking-widest">{t('pmv.assetEditorUnavailable')}</h3>
            <p className="text-zinc-500 text-xs mb-8 uppercase tracking-widest max-w-md text-center">
                {t('pmv.assetEditorUnavailableDesc')}
            </p>
            <button
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 min-h-full flex flex-col font-sans">
      <div className="relative w-full h-80 rounded-[40px] overflow-hidden mb-8 group shrink-0">
        <div className="absolute inset-0"><img src="https://images.unsplash.com/photo-1552820728-8b83bb6b773f?q=80&w=2070&auto=format&fit=crop" className="w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-1000" alt="Valorant Background" /><div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-zinc-950/60 to-transparent" /><div className="absolute inset-0 bg-gradient-to-r from-zinc-950 via-transparent to-transparent" /></div>
        <div className="absolute top-6 left-6 z-20 flex items-center gap-2">
          {isDeployMode && onBackToTerminal && (
            <button onClick={onBackToTerminal} className="flex items-center gap-2 text-xs font-bold text-gray-400 hover:text-white transition-colors bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 hover:bg-black/60"><ArrowLeft size={14} /> {t('pmv.terminal')}</button>
          )}
          <button onClick={onBack} className="flex items-center gap-2 text-xs font-bold text-gray-400 hover:text-white transition-colors bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 hover:bg-black/60"><ArrowLeft size={14} /> {isDeployMode ? t('pmv.backToProject') : t('pmv.backToHub')}</button>
        </div>
        <div className="absolute bottom-8 left-8 z-20 max-w-2xl"><div className="flex items-center gap-3 mb-2 animate-in slide-in-from-left-4 duration-500 delay-100"><div className="bg-[#ccff00] text-black text-[10px] font-black px-2 py-0.5 rounded tracking-widest uppercase">{t('pmv.officialPartner')}</div><div className="bg-amber-500 text-black text-[10px] font-black px-2 py-0.5 rounded tracking-widest uppercase flex items-center gap-1"><Trophy size={10} /> {t('pmv.pmplReady')}</div></div><h1 className="text-5xl md:text-7xl font-black italic tracking-tighter text-white uppercase leading-none mb-4 drop-shadow-2xl animate-in slide-in-from-left-4 duration-500 delay-200">VALO<span className="text-transparent bg-clip-text bg-gradient-to-r from-[#ccff00] to-white">RANT</span></h1><p className="text-gray-400 text-sm font-medium max-w-lg leading-relaxed animate-in slide-in-from-left-4 duration-500 delay-300">{t('pmv.heroDesc')}</p></div>
        <div className="absolute right-8 bottom-8 z-20 flex gap-4 animate-in fade-in duration-700 delay-500 hidden md:flex"><div className="bg-black/60 backdrop-blur-md border border-white/10 p-4 rounded-2xl"><p className="text-[9px] font-black text-gray-500 uppercase tracking-widest mb-1">{t('pmv.liveNodes')}</p><div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#ccff00] animate-pulse" /><span className="text-xl font-black text-white">842</span></div></div><div className="bg-black/60 backdrop-blur-md border border-white/10 p-4 rounded-2xl"><p className="text-[9px] font-black text-gray-500 uppercase tracking-widest mb-1">{t('pmv.activeRegion')}</p><div className="flex items-center gap-2"><Globe size={14} className="text-blue-500" /><span className="text-xl font-black text-white">{t('pmv.global')}</span></div></div></div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4 sticky top-0 bg-zinc-950/95 backdrop-blur z-40 py-4 border-b border-white/5">
        <div className="flex items-center gap-2 bg-zinc-900/50 p-1.5 rounded-xl border border-white/5">{(['TEMPLATES', 'WIDGETS', 'CONFIG'] as const).map((tab) => (<button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-2.5 rounded-lg text-[10px] font-black tracking-[0.2em] uppercase transition-all ${activeTab === tab ? 'bg-[#ccff00] text-black shadow-[0_0_15px_rgba(204,255,0,0.2)]' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}>{t(TAB_LABELS[tab])}</button>))}</div>
        <div className="flex items-center gap-3 w-full md:w-auto"><div className="px-4 py-2 rounded-lg bg-white/5 border border-white/10 flex items-center gap-2 mr-4"><span className="text-[9px] text-gray-500 font-bold uppercase tracking-wider">{userRole === 'admin' ? t('pmv.overrideLabel') : t('pmv.planLabel')}</span><span className="text-xs font-black text-[#ccff00] tracking-widest uppercase">{memberPackage}</span></div><div className="relative group flex-1 md:flex-none"><Search size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-600 group-focus-within:text-[#ccff00] transition-colors" /><input type="text" placeholder={t('pmv.searchAssets')} className="w-full md:w-64 bg-zinc-900 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-xs font-bold text-white placeholder:text-gray-700 focus:outline-none focus:border-[#ccff00]/50 transition-all uppercase tracking-wider" /></div><button className="w-10 h-10 rounded-xl bg-zinc-900 border border-white/10 flex items-center justify-center text-gray-500 hover:text-white hover:border-[#ccff00]/30 transition-all"><Filter size={16} /></button></div>
      </div>

      <div className="relative w-full bg-zinc-900/30 border border-white/5 rounded-[40px] p-8 md:p-10 overflow-hidden pb-20"><div className="absolute inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:24px_24px] pointer-events-none" /><div className="relative z-10 flex items-end gap-4 mb-8 border-b border-white/5 pb-6"><div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-zinc-800 to-black border border-white/10 flex items-center justify-center shadow-lg"><Layout size={32} className="text-[#ccff00]" /></div><div><h2 className="text-3xl font-black italic uppercase text-white tracking-tight leading-none mb-1">{t('pmv.selectLayout')}</h2><p className="text-sm text-gray-500 font-medium">{t('pmv.selectLayoutDesc')}</p></div></div>
