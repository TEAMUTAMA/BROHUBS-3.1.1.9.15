...
type TeamPlacementStatus = Pick<LeaderboardTeamState, 'status'> &
  Partial<Pick<LeaderboardTeamState, 'active' | 'placementRank'>>;

export function teamHasAlivePlayer(team: TeamPlacementStatus): boolean {
  return team.status.some((s) => s === 1);
}

/** Tim sudah keluar dari pertandingan match ini (punya placement atau tidak ada pemain hidup) */
export function isTeamMatchEliminated(team: TeamPlacementStatus): boolean {
