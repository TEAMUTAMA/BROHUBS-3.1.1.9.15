...
import GlobalStudio from '@/features/admin-dashboard/GlobalStudio';
      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col h-full">
        <header className="mb-10">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] font-black text-zinc-500 tracking-[0.4em] uppercase">{t('mbv.broadcastSelectionUnit')}</span>
          </div>
          <h1 className="text-4xl font-black italic text-white uppercase tracking-tighter mb-4">{t('mbv.startHeading')} <span className="text-[#ccff00]">{t('mbv.broadcastWord')}</span></h1>
          <p className="text-gray-500 text-sm max-w-lg leading-relaxed">{t('mbv.selectProjectDesc')}</p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.length === 0 ? (
            <div className="col-span-full py-24 text-center border-2 border-dashed border-white/5 rounded-[40px] bg-zinc-900/10">
                ...
                <h3 className="text-zinc-600 font-black tracking-widest uppercase text-sm mb-1">{t('mbv.noProjectsFound')}</h3>
                <p className="text-zinc-700 text-xs">{t('mbv.createProjectHint')}</p>
            </div>
          ) : (
            projects.map((project) => (
              ...
              >
                ...
                <p className="text-xs text-zinc-600 font-medium line-clamp-1 mb-8 uppercase tracking-widest">{t('mbv.assetsDeployed').replace('{count}', String(project.deployedAssets?.length || 0))}</p>
                ...
                <div className="flex items-center justify-between pt-6 border-t border-white/5">
                    ...
                    <button className="flex items-center gap-2 text-[10px] font-black text-[#ccff00] uppercase tracking-widest group-hover:translate-x-2 transition-transform">
                        {t('mbv.initialize')} <ChevronRight size={14} strokeWidth={3} />
                    </button>
                </div>
              </div>
            ))
    ...
    <div className="animate-in fade-in slide-in-from-right-8 duration-500 flex flex-col h-full">
      <header className="mb-10 flex flex-col md:flex-row items-start md:items-end justify-between gap-6">
        <div>
          ...
          >
            <ArrowLeft size={12} /> {t('mbv.changeProject')}
          </button>
          <div className="flex items-center gap-4">
            ...
            <div>
                <h1 className="text-3xl font-black italic text-white uppercase tracking-tighter leading-none mb-2">
                    {t('mbv.broadcastConsoleHeading')} <span className="text-[#ccff00]">{t('mbv.consoleWord')}</span>
                </h1>
                <div className="flex items-center gap-3">
                    <span className="px-2 py-0.5 rounded bg-white/5 border border-white/10 text-[8px] font-black text-zinc-500 uppercase tracking-widest">{t('mbv.activeSession')}</span>
        ...
        <div className="flex items-center gap-4 bg-zinc-900/50 border border-white/5 p-4 rounded-3xl">
             <div className="flex flex-col items-end border-r border-white/5 pr-4">
                <span className="text-[7px] font-black text-zinc-600 uppercase tracking-widest">{t('mbv.sessionUplink')}</span>
                <span className="text-[#ccff00] text-[10px] font-black tracking-widest uppercase">STABLE // 4K</span>
             </div>
             <div className="flex flex-col items-end">
                <span className="text-[7px] font-black text-zinc-600 uppercase tracking-widest">{t('mbv.activeNodes')}</span>
                <span className="text-white text-[10px] font-black tracking-widest uppercase">{t('mbv.deployedCount').replace('{count}', String(selectedProject.deployedAssets?.length || 0))}</span>
             </div>
        </div>
      </header>

      <div className="flex-1">
        <h2 className="text-[11px] font-black text-zinc-500 tracking-[0.3em] uppercase mb-6 border-b border-white/5 pb-4">{t('mbv.operationalAssets')}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pb-20">
            {(selectedProject.deployedAssets || []).length > 0 ? (
                (selectedProject.deployedAssets || []).map((asset) => {
                    ...
                    >
                        ...
                        <button className={`w-full py-3 ${accessible ? 'bg-[#ccff00]/10 border-[#ccff00]/20 text-[#ccff00] hover:bg-[#ccff00] hover:text-black' : 'bg-zinc-800 border-zinc-700 text-zinc-500'} rounded-xl text-[9px] font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2`}>
                            {accessible ? <><Settings size={12} /> {t('mbv.configureNode')}</> : t('mbv.lockedUpgrade')}
                        </button>
                ...
                <div className="col-span-full py-12 text-center border border-dashed border-zinc-800 rounded-[32px] opacity-40">
                    <p className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">{t('mbv.noAssetsForTier')}</p>
                </div>
            )}
        </div>
      </div>
      ...
      <footer className="mt-auto py-8 border-t border-white/5 flex items-center justify-between opacity-30">
          <div className="flex items-center gap-3">
              ...
              <span className="text-[9px] font-black text-white uppercase tracking-widest">{t('mbv.operationalConsoleVersion')}</span>
          </div>
          <div className="flex items-center gap-2">
              <span className="text-[8px] font-bold text-zinc-600 uppercase tracking-widest">{t('mbv.systemHealth')}</span>
              <span className="text-[8px] font-black text-[#ccff00] uppercase tracking-widest">{t('mbv.systemHealthOptimal')}</span>
          </div>
      </footer>
    </div>
  );
