    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/5 pb-8 gap-6">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h2 className="text-2xl font-bold tracking-tight text-white">{t('mov.outputHubTitle').replace('{name}', povMember.name)}</h2>
            <span className="px-2 py-0.5 bg-[#c1ff00] text-black text-[9px] font-black uppercase rounded-md shadow-[0_0_10px_rgba(193,255,0,0.3)]">{t('mov.memberNode')}</span>
          </div>
          <p className="text-white/30 text-sm">{t('mov.subtitleTransmission')}</p>
        </div>
        <div className="px-4 py-2 bg-blue-500/10 border border-blue-500/20 rounded-full flex items-center gap-2">
          <div className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse"></div>
          <span className="text-[10px] font-bold text-blue-400 uppercase tracking-widest">{t('mov.activeSignalTracking')}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between px-1">
          <span className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em]">{t('mov.projectRepositoryFilter')}</span>
          <span className="text-[8px] font-mono text-[#c1ff00]/40 uppercase">{t('mov.totalNodes').replace('{count}', String(filteredAssets.length))}</span>
        </div>
        <div className="flex items-center gap-4">
          <div className="relative flex-1">
            <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="w-full bg-white/5 border border-white/5 rounded-xl p-4 text-[10px] font-black uppercase text-white tracking-widest flex items-center justify-between hover:border-white/20 transition-all">
                {activeFilter === 'all' ? t('mov.allProjects') : memberProjectsWithAssets.find(p => p.id === activeFilter)?.name || t('mov.selectProject')}
                <ChevronDown size={16} />
            </button>
            {isDropdownOpen && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-[#111] border border-white/10 rounded-xl overflow-hidden z-20 shadow-2xl">
                    <button onClick={() => { setActiveFilter('all'); setIsDropdownOpen(false); }} className="w-full text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest hover:bg-white/5">{t('mov.allProjects')}</button>
          ...
          <div className="flex-1">
              <input 
                  type="text" 
                  placeholder={t('mov.searchProjectLink')}
                  value={searchQuery}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredAssets.length > 0 ? (
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="flex items-center">
                        <div className="px-3 py-1 bg-white/10 rounded-lg text-[10px] font-black text-[#c1ff00] uppercase tracking-[0.2em] border border-white/5 shadow-inner">
                           {t('mov.folderLabel').replace('{folder}', project?.name || t('mov.rootFolder'))}
                        </div>
                    </div>
                ...
                <div className="space-y-4 relative z-10">
                  <div className="bg-black/60 border border-white/10 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 group/input relative overflow-hidden hover:border-white/20 transition-all">
                    <div className="flex flex-col min-w-0">
                       <span className="text-[8px] font-black text-white/20 uppercase tracking-widest mb-1">{t('mov.tripleSegmentEndpoint')}</span>
                    ...
                    >
                      {copyFeedback === publicUrl ? t('mov.copied') : t('mov.copyUrl')}
                    </button>
                  </div>
                </div>
              </div>
          ...
          <div className="col-span-full py-24 text-center bg-white/[0.02] border border-dashed border-white/10 rounded-[2.5rem]">
            <p className="text-white/20 font-bold uppercase tracking-[0.4em] text-xs">{t('mov.noActiveNodes')}</p>
            <button
              onClick={() => setActiveFilter('all')}
              className="mt-4 text-[10px] font-black text-[#c1ff00] uppercase tracking-widest hover:underline"
            >
              {t('mov.resetAllTransmissions')}
            </button>
          </div>
        )}
      </div>
