...
const TeamListManagement: React.FC<TeamListManagementProps> = ({ players, onUpdatePlayers, onEditTeam, onArchiveTeam, onClose }) => {
  ...
  const handleDeleteTeam = (teamName: string) => {
    if (onArchiveTeam) {
        ...
        if (window.confirm(t('tlm.confirmDeleteTeam').replace('{team}', teamName.toUpperCase()))) {
      ...
      <div className="relative w-full max-w-5xl bg-[#0a0a0a] border border-white/5 rounded-3xl md:rounded-[48px] overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95">
        ...
        <div className="p-6 md:p-10 pb-4 md:pb-6 flex justify-between items-start shrink-0">
          <div>
            <h2 className="text-xl md:text-4xl font-[1000] italic text-white uppercase tracking-tighter leading-none mb-2">
              {t('tlm.headingTeam')} <span className="text-blue-500">{t('tlm.headingDatabase')}</span>
            </h2>
            <p className="text-[7px] md:text-[10px] font-black text-zinc-600 uppercase tracking-[0.4em]">{t('tlm.storedPersonnelArchive')}</p>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-10 pt-0 custom-scrollbar space-y-6 md:space-y-10">
          ...
          {teamList.length === 0 ? (
            <div className="py-20 flex flex-col items-center text-center opacity-40">
                ...
                <p className="text-xs font-black uppercase tracking-[0.3em] text-zinc-600">{t('tlm.noTeamData')}</p>
            </div>
          ) : (
                  ...
                  <div className="p-4 md:p-6 bg-white/5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[8px] md:text-[9px] font-black text-blue-500 uppercase tracking-widest mt-1">{t('tlm.personnelDeployed').replace('{count}', String(team.players.length))}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 w-full md:w-auto">
                        ...
                        >
                            <Edit3 size={12} strokeWidth={3} /> {t('tlm.updateBtn')} <span className="md:hidden">{t('tlm.updateBtnData')}</span>
                        </button>
                        <button 
                            ...
                            title={onArchiveTeam ? t('tlm.titleArchiveTeam') : t('tlm.titleDeleteTeam')}
                        >
                            <Trash2 size={18} />
                        </button>
                    </div>
        ...
        <div className="p-6 md:p-8 border-t border-white/5 bg-zinc-950 flex flex-col md:flex-row justify-between items-center shrink-0 gap-4">
             <div className="flex items-center gap-3">
                ...
                <p className="text-[8px] md:text-[9px] font-black text-zinc-500 uppercase tracking-widest">{t('tlm.footerTotal').replace('{players}', String(players.length)).replace('{teams}', String(teamList.length))}</p>
             </div>
             <button 
               onClick={onClose}
